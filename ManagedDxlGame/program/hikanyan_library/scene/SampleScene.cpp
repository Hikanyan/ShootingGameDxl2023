...
void SampleScene::init()
{
    ...
    // その他のオブジェクト固有の設定
    if (auto transform = obj_->get_transform().lock())
    {
        transform->set_position(tnl::Vector3(100, 100, 0));
        transform->set_scale(tnl::Vector3(100, 100, 0));
    }
    ...
    if (auto transform = obj_->get_transform().lock())
    {
        transform->set_position(tnl::Vector3(200, 200, 0));
        transform->set_scale(tnl::Vector3(100, 100, 0));
    }
